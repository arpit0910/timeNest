...
@section('content')
    ...
    <main x-data="{ searchQuery: '', activeCategory: 'All' }" class="flex-grow pb-20 bg-slate-950 relative overflow-hidden">
        {{-- Section 1: Hero --}}
        ...
        <section class="py-24 bg-slate-950 border-y border-slate-800"><div class="px-6 max-w-7xl mx-auto">
            <div class="text-center max-w-3xl mx-auto mb-12">
                ...
                <p class="text-lg text-slate-400">Catch up on our newest thoughts and strategies for modern workforce management.</p>
                <div class="mt-10">
                    {{-- Search Input --}}
                <div class="relative max-w-lg mx-auto mb-6">
                    ...
                    <input type="text" x-model="searchQuery" class="block w-full pl-12 pr-4 py-3 border border-white/10 rounded-full leading-5 bg-white/5 placeholder-slate-500 text-white focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-accent-500 sm:text-sm shadow-sm transition-shadow hover:shadow-md" placeholder="Search articles by keyword...">
                </div>

                {{-- Category Filters --}}
                <div class="flex flex-wrap justify-center gap-2">
                    <button @click="activeCategory = 'All'" :class="activeCategory === 'All' ? 'bg-accent-600 text-white shadow-md' : 'bg-white/5 text-slate-300 border border-white/10 hover:border-white/20 hover:text-white transition-colors shadow-sm'" class="px-4 py-1.5 rounded-full text-sm font-semibold">All</button>
                    <button @click="activeCategory = 'Workforce Management'" :class="activeCategory === 'Workforce Management' ? 'bg-accent-600 text-white shadow-md' : 'bg-white/5 text-slate-300 border border-white/10 hover:border-white/20 hover:text-white transition-colors shadow-sm'" class="px-4 py-1.5 rounded-full text-sm font-semibold">Workforce Management</button>
                    <button @click="activeCategory = 'Security & Compliance'" :class="activeCategory === 'Security & Compliance' ? 'bg-accent-600 text-white shadow-md' : 'bg-white/5 text-slate-300 border border-white/10 hover:border-white/20 hover:text-white transition-colors shadow-sm'" class="px-4 py-1.5 rounded-full text-sm font-semibold">Security & Compliance</button>
                    <button @click="activeCategory = 'Growing Teams'" :class="activeCategory === 'Growing Teams' ? 'bg-accent-600 text-white shadow-md' : 'bg-white/5 text-slate-300 border border-white/10 hover:border-white/20 hover:text-white transition-colors shadow-sm'" class="px-4 py-1.5 rounded-full text-sm font-semibold">Growing Teams</button>
                    <button @click="activeCategory = 'Remote & Hybrid Work'" :class="activeCategory === 'Remote & Hybrid Work' ? 'bg-accent-600 text-white shadow-md' : 'bg-white/5 text-slate-300 border border-white/10 hover:border-white/20 hover:text-white transition-colors shadow-sm'" class="px-4 py-1.5 rounded-full text-sm font-semibold">Remote & Hybrid Work</button>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {{-- Grid Post 1 --}}
                <a href="#" x-show="(activeCategory === 'All' || activeCategory === 'Workforce Management') && (searchQuery === '' || $el.innerText.toLowerCase().includes(searchQuery.toLowerCase()))" class="post-card group bg-slate-900 rounded-3xl overflow-hidden shadow-none border border-white/10 hover:border-white/20 transition-all duration-300 flex flex-col h-full">
                    ...
                    <div class="p-6 md:p-8 flex flex-col flex-grow items-start">
                        ...
                        <p class="text-slate-400 text-sm mb-6 flex-grow">What actually gets lost when approvals live in someone's inbox.</p>
                        <div class="text-sm font-medium text-slate-400 mt-auto pt-4 border-t border-white/10 w-full">TimeNest Team</div>
                    </div>
                </a>
                ...
                <a href="#" x-show="(activeCategory === 'All' || activeCategory === 'Security & Compliance') && (searchQuery === '' || $el.innerText.toLowerCase().includes(searchQuery.toLowerCase()))" class="post-card group bg-slate-900 rounded-3xl overflow-hidden shadow-none border border-white/10 hover:border-white/20 transition-all duration-300 flex flex-col h-full">
                    ...
                    <div class="p-6 md:p-8 flex flex-col flex-grow items-start">
                        ...
                        <p class="text-slate-400 text-sm mb-6 flex-grow">A plain-language look at location-verified check-ins.</p>
                        <div class="text-sm font-medium text-slate-400 mt-auto pt-4 border-t border-white/10 w-full">TimeNest Team</div>
                    </div>
                </a>

                {{-- Grid Post 3 --}}
                <a href="#" x-show="(activeCategory === 'All' || activeCategory === 'Growing Teams') && (searchQuery === '' || $el.innerText.toLowerCase().includes(searchQuery.toLowerCase()))" class="post-card group bg-slate-900 rounded-3xl overflow-hidden shadow-none border border-white/10 hover:border-white/20 transition-all duration-300 flex flex-col h-full">
                    <div class="h-52 overflow-hidden bg-slate-100">
                    ...
                    <div class="p-6 md:p-8 flex flex-col flex-grow items-start">
                        ...
                        <p class="text-slate-400 text-sm mb-6 flex-grow">The signs it's time to move past a solo setup.</p>
                        <div class="text-sm font-medium text-slate-400 mt-auto pt-4 border-t border-white/10 w-full">TimeNest Team</div>
                    </div>
                </a>
                ...
                <a href="#" x-show="(activeCategory === 'All' || activeCategory === 'Security & Compliance') && (searchQuery === '' || $el.innerText.toLowerCase().includes(searchQuery.toLowerCase()))" class="post-card group bg-slate-900 rounded-3xl overflow-hidden shadow-none border border-white/10 hover:border-white/20 transition-all duration-300 flex flex-col h-full">
                    ...
                    <div class="p-6 md:p-8 flex flex-col flex-grow items-start">
                        ...
                        <p class="text-slate-400 text-sm mb-6 flex-grow">What 2FA actually protects against, without the jargon.</p>
                        <div class="text-sm font-medium text-slate-400 mt-auto pt-4 border-t border-white/10 w-full">TimeNest Team</div>
                    </div>
                </a>
                ...
                <a href="#" x-show="(activeCategory === 'All' || activeCategory === 'Remote & Hybrid Work') && (searchQuery === '' || $el.innerText.toLowerCase().includes(searchQuery.toLowerCase()))" class="post-card group bg-slate-900 rounded-3xl overflow-hidden shadow-none border border-white/10 hover:border-white/20 transition-all duration-300 flex flex-col h-full">
                    ...
                    <div class="p-6 md:p-8 flex flex-col flex-grow items-start">
                        ...
                        <p class="text-slate-400 text-sm mb-6 flex-grow">Why the "just use WhatsApp" phase has a shelf life.</p>
                        <div class="text-sm font-medium text-slate-400 mt-auto pt-4 border-t border-white/10 w-full">TimeNest Team</div>
                    </div>
                </a>
                ...
                <a href="#" x-show="(activeCategory === 'All' || activeCategory === 'Workforce Management') && (searchQuery === '' || $el.innerText.toLowerCase().includes(searchQuery.toLowerCase()))" class="post-card group bg-slate-900 rounded-3xl overflow-hidden shadow-none border border-white/10 hover:border-white/20 transition-all duration-300 flex flex-col h-full">
                    ...
                    <div class="p-6 md:p-8 flex flex-col flex-grow items-start">
                        ...
                        <p class="text-slate-400 text-sm mb-6 flex-grow">Access control that protects data without slowing people down.</p>
                        <div class="text-sm font-medium text-slate-400 mt-auto pt-4 border-t border-white/10 w-full">TimeNest Team</div>
                    </div>
                </a>
            </div>
        ...
        <section class="py-24 bg-white border-y border-slate-100"><div class="px-6 max-w-7xl mx-auto">
            <div class="max-w-7xl mx-auto">
                <div class="text-center max-w-3xl mx-auto mb-16">
                    ...
                    <h2 class="text-3xl md:text-4xl font-bold text-slate-900 mb-4">Featured Article</h2>
                    <p class="text-lg text-slate-600">The most important piece of writing from our team this month.</p>
                </div>
                <a href="#" class="block group relative bg-white rounded-[2.5rem] overflow-hidden shadow-xl border border-slate-100 hover:shadow-2xl transition-all duration-300">
        ...
        <section class="py-24 bg-slate-950 border-y border-slate-800">
            <div class="max-w-7xl mx-auto px-6">
                <div class="text-center max-w-3xl mx-auto mb-16">
                    ...
                    <p class="text-lg text-slate-400">The articles that have resonated most with leaders and managers.</p>
                </div>
                
                {{-- Flex Wrap Grid Layout --}}
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {{-- Popular Post 1 --}}
                    <a href="#" class="flex flex-col bg-slate-900 rounded-3xl shadow-none border border-white/10 p-8 hover:border-white/20 transition-all group hover:-translate-y-2 duration-300">
                        ...
                        <p class="text-slate-400 text-sm mb-6 flex-grow">The structural shift most growing teams don't see coming.</p>
                    ...
                    <a href="#" class="flex flex-col bg-slate-900 rounded-3xl shadow-none border border-white/10 p-8 hover:border-white/20 transition-all group hover:-translate-y-2 duration-300">
                        ...
                        <p class="text-slate-400 text-sm mb-6 flex-grow">Why routing requests through a real hierarchy beats a single bottleneck.</p>
                        <div class="text-sm font-medium text-slate-400 mt-auto pt-4 border-t border-slate-100">TimeNest Team</div>
                    </a>
        ...
        <section class="py-24 bg-white border-y border-slate-100"><div class="px-6 max-w-7xl mx-auto">
            <div class="text-center max-w-3xl mx-auto mb-16">
                ...
                <h2 class="text-3xl md:text-4xl font-bold text-slate-900 mb-4">Deep Dives</h2>
                <p class="text-lg text-slate-600">Comprehensive guides and technical perspectives for power users.</p>
            </div>
        ...
        <section class="py-24 bg-slate-950 border-y border-slate-800"><div class="max-w-4xl mx-auto px-6">
            <div class="text-center max-w-3xl mx-auto mb-12">
                ...
                <p class="text-lg text-slate-400">Short takes and updates that only take a minute.</p>
            </div>
            <div class="bg-slate-900 rounded-3xl shadow-none border border-white/10 overflow-hidden divide-y divide-white/10">
