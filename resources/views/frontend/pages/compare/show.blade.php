...
<x-frontend-layout.app :metaTitle="'TimeNest vs ' . $data['name'] . ' â€” Comparison'" :metaDescription="$data['verdict']">
    <section class="relative pt-32 pb-20 bg-surface">
        <div class="max-w-4xl mx-auto px-6 lg:px-8">
            <h1 class="font-display text-4xl lg:text-5xl font-bold text-white mb-4">TimeNest vs {{ $data['name'] }}</h1>
            <p class="text-slate-400 text-lg mb-12">{{ $data['verdict'] }}</p>

            <div class="rounded-xl border border-surface-border overflow-hidden mb-12">
                <table class="w-full text-sm">
                    <thead><tr class="bg-surface-card"><th class="p-4 text-left text-slate-400">Feature</th><th class="p-4 text-center text-brand-400">TimeNest</th><th class="p-4 text-center text-slate-400">{{ $data['name'] }}</th></tr></thead>
                    <tbody>
                        @foreach([
                            ...
                            <tr class="border-t border-surface-border">
                                ...
                                <td class="p-4 text-center">@if($comp)<svg class="w-5 h-5 text-slate-500 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>@else<span class="text-slate-600">â€”</span>@endif</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
