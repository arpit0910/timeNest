<footer class="bg-surface border-t border-surface-border">
        ...
        <div class="grid grid-cols-2 md:grid-cols-6 gap-8">
            ...
            <div class="col-span-2 md:col-span-1">
                <a href="{{ route('frontend.home') }}" class="flex items-center gap-2 mb-4">
                    ...
                    <span class="font-display text-lg font-bold text-content-strong">Time<span class="text-brand-400">Nest</span></span>
                </a>
                <p class="text-content-muted text-sm mb-4">The Work Operating System for modern teams.</p>
            </div>

            {{-- Product --}}
            <div>
                <h4 class="font-display font-semibold text-content-strong text-sm mb-4">Product</h4>
                <ul class="space-y-2">
                    @foreach([
                        ...
                        <li><a href="{{ route($routeName) }}" class="text-content-muted text-sm hover:text-content-strong transition-colors">{{ $label }}</a></li>
                    @endforeach
                </ul>
            </div>
            ...
            {{-- Solutions --}}
            <div>
                <h4 class="font-display font-semibold text-content-strong text-sm mb-4">Solutions</h4>
                <ul class="space-y-2">
                    @foreach([
                        ...
                        <li><a href="{{ route('frontend.solutions.show', $slug) }}" class="text-content-muted text-sm hover:text-content-strong transition-colors">{{ $label }}</a></li>
                    @endforeach
                </ul>
            </div>
            ...
            {{-- Company --}}
            <div>
                <h4 class="font-display font-semibold text-content-strong text-sm mb-4">Company</h4>
                <ul class="space-y-2">
                    @foreach([
            ...
            {{-- Resources --}}
            <div>
                <h4 class="font-display font-semibold text-content-strong text-sm mb-4">Resources</h4>
                <ul class="space-y-2">
                    @foreach([
            ...
            {{-- Legal --}}
            <div>
                <h4 class="font-display font-semibold text-content-strong text-sm mb-4">Legal</h4>
                <ul class="space-y-2">
                    @foreach([
    ...
    <div class="border-t border-surface-border">
        <div class="max-w-7xl mx-auto px-6 lg:px-8 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            <p class="text-content-light text-sm">&copy; {{ date('Y') }} TimeNest. All rights reserved.</p>
            <p class="text-content-light text-sm">Made in India ðŸ‡®ðŸ‡³</p>
        </div>
    </div>
</footer>
