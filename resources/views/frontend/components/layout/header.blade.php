...
>
    <div class="max-w-7xl mx-auto px-6 lg:px-8">
        <div class="flex items-center justify-between h-16 lg:h-20">
            <a href="{{ route('frontend.home') }}" class="flex items-center gap-2">
                ...
                <span class="font-display text-xl font-bold text-white">Time<span class="text-brand-400">Nest</span></span>
            </a>

            <nav class="hidden lg:flex items-center gap-1">
                @foreach(['products', 'solutions', 'ai', 'resources', 'company'] as $menu)
                    <div @mouseenter="activeMenu = '{{ $menu }}'">
                        <button class="px-3 py-2 text-sm text-slate-300 hover:text-white font-body flex items-center gap-1 transition-colors cursor-pointer">
                            {{ ucfirst($menu) }}
                ...
                @endforeach
                <a href="{{ route('frontend.pricing') }}" class="px-3 py-2 text-sm text-slate-300 hover:text-white font-body transition-colors" @mouseenter="activeMenu = null">Pricing</a>
                <a href="{{ route('frontend.roadmap') }}" class="px-3 py-2 text-sm text-slate-300 hover:text-white font-body transition-colors" @mouseenter="activeMenu = null">Roadmap</a>
            </nav>

            <div class="hidden lg:flex items-center gap-3">
                <button @click="$store.search.toggle()" class="p-2 text-slate-400 hover:text-white transition-colors cursor-pointer">
            ...
            <button @click="$store.mobileNav.toggle()" class="lg:hidden p-2 text-slate-400 hover:text-white cursor-pointer">
    ...
    {{-- Mega Menu --}}
    <div x-show="activeMenu" x-transition x-cloak class="absolute top-full left-0 right-0 bg-surface-card/95 glass border-b border-surface-border">
    ...
    <div x-show="$store.mobileNav.open" x-transition x-cloak class="lg:hidden bg-surface-card border-b border-surface-border">
        <div class="px-6 py-4 space-y-3">
            @foreach([
                ...
                <a href="{{ route($routeName) }}" class="block text-slate-300 hover:text-white text-sm py-2 font-body">{{ $label }}</a>
            @endforeach
