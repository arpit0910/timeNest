...
class="relative w-full max-w-6xl mx-auto">
    ...
    {{-- Main card --}}
    <div class="relative rounded-2xl sm:rounded-3xl bg-white border border-slate-200/60 shadow-lg shadow-slate-200/50 overflow-hidden">
            ...
            @foreach($slides as $index => $slide)
                <div x-show="activeSlide === {{ $index }}"
                    ...
                    <div class="flex flex-col md:flex-row">
                        ...
                        <div class="relative w-full md:w-[50%] lg:w-[52%] bg-gradient-to-br from-slate-50 via-indigo-50/40 to-brand-50/30 shrink-0">
                            ...
                            <div class="relative flex items-center justify-center p-5 sm:p-8 md:p-8 lg:p-10 min-h-[220px] sm:min-h-[280px] md:min-h-[360px]">
                                <div class="relative group/img w-full max-w-md">
                                    ...
                                    <img src="{{ $slide['image'] }}" 
                                         alt="{{ $slide['title'] }}" 
                                         class="relative w-full h-auto object-contain rounded-xl shadow-md shadow-slate-900/8 border border-white/80 bg-white/60 backdrop-blur-sm ring-1 ring-slate-900/[0.04] hover:shadow-lg hover:scale-[1.02] transition-all duration-500">
                                </div>
                            </div>

                            {{-- Decorative accent line --}}
                            <div class="hidden md:block absolute right-0 top-[15%] bottom-[15%] w-px bg-gradient-to-b from-transparent via-slate-200/80 to-transparent"></div>
                        </div>
                        ...
                        <div class="w-full md:w-[50%] lg:w-[48%] p-5 sm:p-8 md:p-8 lg:p-12 flex flex-col justify-center">
                            ...
                            {{-- Title --}}
                            <h3 class="font-display text-lg sm:text-xl lg:text-2xl xl:text-[1.7rem] font-bold text-slate-900 mb-3 leading-[1.2] tracking-tight">{{ $slide['title'] }}</h3>
                            
                            {{-- Divider --}}
                            <div class="w-12 h-0.5 bg-gradient-to-r from-brand-400 to-indigo-400 rounded-full mb-3 sm:mb-4"></div>

                            {{-- Description --}}
                            <p class="text-slate-500 text-[13px] sm:text-sm lg:text-[15px] leading-relaxed mb-4 sm:mb-5">{{ $slide['description'] }}</p>

                            {{-- Keyword badges --}}
                            @if(isset($slide['tags']))
                                <div class="flex flex-wrap gap-1.5 mb-5 sm:mb-6">
                                    @foreach($slide['tags'] as $tag)
                                        <span class="text-[10px] sm:text-[11px] font-semibold px-2.5 py-1 rounded-full bg-slate-50 text-slate-600 border border-slate-200/60">{{ $tag }}</span>
                                    @endforeach
                                </div>
                            @endif
        ...
        {{-- Bottom controls bar --}}
        <div class="relative border-t border-slate-100 bg-slate-50/50">
            <div class="flex items-center justify-between px-4 sm:px-6 md:px-8 py-3 sm:py-3.5">
                {{-- Slide counter --}}
                <div class="text-xs font-medium text-slate-400 tracking-wider tabular-nums">
                    <span class="text-slate-700" x-text="String(activeSlide + 1).padStart(2, '0')">01</span>
                ...
                <div class="flex items-center gap-1.5">
                    <button @click="prev()" 
                            class="w-8 h-8 sm:w-9 sm:h-9 rounded-lg flex items-center justify-center text-slate-400 hover:text-slate-700 hover:bg-white hover:shadow-sm border border-transparent hover:border-slate-200 transition-all duration-200">
