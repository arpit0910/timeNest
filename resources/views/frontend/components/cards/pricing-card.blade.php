<div class="relative rounded-xl border {{ $highlighted ? 'border-brand-500 bg-surface-card shadow-xl shadow-brand-500/10' : 'border-surface-border bg-surface-card' }} p-8 flex flex-col {{ $class }}">
    @if($badge)
        <div class="absolute -top-3 left-1/2 -translate-x-1/2">
            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-brand-500 text-white">{{ $badge }}</span>
        </div>
    @endif

    <div class="mb-6">
        <h3 class="font-display text-xl font-bold text-white mb-2">{{ $name }}</h3>
        <p class="text-slate-400 text-sm">{{ $description }}</p>
    </div>

    <div class="mb-6">
        <span class="font-display text-4xl font-bold text-white">{{ $price }}</span>
        @if($period)
