    ...
    <div class="max-w-3xl mx-auto px-6 pt-24">
        <div class="flex items-center gap-4 mb-8">
            <div class="flex-1 relative">
                ...
                <input id="global-search-input" type="text" placeholder="Search pages, features, FAQs..." class="w-full pl-12 pr-4 py-4 bg-surface-card border border-surface-border rounded-xl text-white placeholder-slate-500 font-body focus:outline-none focus:border-brand-500" x-model="$store.search.query" @input="$store.search.search($event.target.value)" @keydown.escape="$store.search.close()">
            </div>
            <button @click="$store.search.close()" class="p-2 text-slate-400 hover:text-white cursor-pointer">
            ...
            <template x-for="result in $store.search.results" :key="result.id">
                <a :href="result.url" class="block p-4 rounded-xl bg-surface-card border border-surface-border hover:border-brand-500/30 transition-colors">
                    <div class="flex items-start justify-between">
                        <div><h3 class="text-white font-medium text-sm" x-text="result.title"></h3><p class="text-slate-400 text-xs mt-1" x-text="result.description"></p></div>
        ...
        <div x-show="$store.search.query.length < 2 && !$store.search.loading" class="text-center py-12">
            <p class="text-slate-500 text-sm">Start typing to search...</p>
            <div class="flex items-center justify-center gap-2 mt-4 text-xs text-slate-600">
                <kbd class="px-2 py-1 rounded bg-surface-card border border-surface-border text-slate-400">ESC</kbd>
                <span>to close</span>
            </div>
        </div>
    </div>
