...
>
    <div class="flex items-center justify-between">
        <span class="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Tasks</span>
    ...
    <div class="mt-2 space-y-1.5 flex-1 flex flex-col justify-center">
        <template x-for="item in items" :key="item.id">
            <div class="flex items-center justify-between text-[10px] transition-all duration-300">
                <span :class="item.done ? 'text-slate-400 line-through' : 'text-slate-700 font-semibold'" x-text="item.name"></span>
                <div :class="'w-3.5 h-3.5 rounded border flex items-center justify-center transition-all duration-300 ' + 
                     (item.done ? 'bg-indigo-500 border-indigo-500 text-white' : 'border-slate-300 bg-white')">
