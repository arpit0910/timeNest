...
class LeaveManagementService
{
     ...
     * Approve leave.
     */
    public function approveLeave(EmployeeLeave $leave, User $actor): EmployeeLeave
    {
     ...
     * Reject leave.
     */
    public function rejectLeave(EmployeeLeave $leave, User $actor, string $reason): EmployeeLeave
    {
        return DB::transaction(function () use ($leave, $actor, $reason) {
            $leave->update([
                'leave_status' => LeaveStatusEnum::Rejected->value,
     ...
     * Cancel leave.
     */
    public function cancelLeave(EmployeeLeave $leave, string $reason): EmployeeLeave
    {
        if (! in_array($leave->leave_status, [LeaveStatusEnum::Pending, LeaveStatusEnum::Approved, LeaveStatusEnum::AutoApproved], true)) {
            throw new BusinessRuleViolationException('Only active or pending leaves can be cancelled.', 'LEAVE_NOT_CANCELLABLE');
        }
