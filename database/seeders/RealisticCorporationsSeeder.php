...
class RealisticCorporationsSeeder extends Seeder
{
    public function run(): void
    {
        ...
        DB::transaction(function () use ($faker, $password, $corpRoles) {
            for ($i = 0; $i < 5; $i++) {
                ...
                // Set team id for Spatie permissions to scope roles to this corp
                setPermissionsTeamId($corp->id);

                foreach ($corpRoles as $roleName) {
                    $role = Role::where('name', $roleName)->whereNull('corporation_id')->first();
                    if (!$role) continue;
        ...
        $this->command->info('Seeded 5 realistic corporations with role-based users.');
    }
}
